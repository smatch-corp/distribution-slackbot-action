...
import { createDirectMessageToActor, createThreadMainMessageSurface } from './messages';
export async function main() {
    try {
        ...
        if (inputs.phase === 'start') {
            ...
            core.info(dedent `Start message sent Successfully: ${JSON.stringify(messageResponse, null, 2)}`);
            if (messageResponse.ts) {
                const permaLink = await slackClient.chat.getPermalink({
                    channel: inputs.channel_id,
                    message_ts: messageResponse.ts
                });
                const directMessageResponse = await slackClient.chat.postMessage(createDirectMessageToActor(permaLink.permalink));
                core.info(dedent `Direct message sent Successfully: ${JSON.stringify(directMessageResponse, null, 2)}`);
            }
        }
